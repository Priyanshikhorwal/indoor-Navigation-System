...
import { Lock, ShieldAlert, UserPlus, ArrowRight, Eye, EyeOff } from 'lucide-react';

const AdminLogin = ({ mode = 'login' }) => {
    const navigate = useNavigate();
    const [activeTab, setActiveTab] = useState(mode === 'register' ? 'register' : 'admin');
    ...
    // Sync tab state with route transitions
    useEffect(() => {
        setActiveTab(mode === 'register' ? 'register' : 'admin');
        setError('');
        setSuccess('');
    }, [mode]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (activeTab === 'register') {
                // Call backend signup API for administrative users (ROLE_ADMIN)
                const response = await api.post('/auth/register', { email, password, role: 'ROLE_ADMIN' });
                ...
                if (userRole === 'ROLE_ADMIN') {
                    setSuccess('Admin authenticated! Redirecting to Console...');
                    setTimeout(() => {
                        navigate('/admin-dashboard');
                    }, 1200);
                } else {
            ...
            console.error('Administrative Authentication Error:', err);
            setError(err.response?.data?.error || err.response?.data?.message || 'Authentication failed. Please verify admin credentials.');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="bg-white dark:bg-darkCard rounded-3xl shadow-soft-lg dark:shadow-none w-full max-w-lg border border-gray-100 dark:border-darkHover overflow-hidden relative z-10 animate-in fade-in zoom-in-95 duration-300">
                ...
                <div className="bg-gradient-to-r from-primary to-primary-light dark:from-darkCard dark:to-darkBg text-white p-8 text-center relative dark:border-b dark:border-darkHover">
                    ...
                    <div className="mx-auto bg-white/10 w-16 h-16 rounded-2xl flex items-center justify-center mb-4 backdrop-blur shadow-inner">
                        {activeTab === 'register' ? (
                            <UserPlus className="w-8 h-8 text-accent animate-pulse" />
                        ) : (
                            <ShieldAlert className="w-8 h-8 text-accent animate-pulse" />
                        )}
                    </div>
                    <h2 className="text-2xl font-bold tracking-tight">
                        {activeTab === 'register' ? 'Admin Console Registration' : 'Administrative Access'}
                    </h2>
                    <p className="text-gray-300 text-xs mt-1">
                        {activeTab === 'register' ? 'Register a new administrator for map and building routing services' : 'Secure gateway for authorized building and map administrators'}
                    </p>
                </div>
                        ...
                        {activeTab === 'register' ? (
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                <div className="pt-2 border-t border-gray-100/50 dark:border-darkHover mt-1 flex justify-center items-center gap-1.5 text-[11px] text-gray-400">
                                    <span>Regular standard user?</span>
                                    <Link to="/login" className="text-primary dark:text-white font-bold hover:underline">
